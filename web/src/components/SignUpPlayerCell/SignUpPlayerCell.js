...
export const Success = ({ schedule, activeSignups, users }) => {
  ...
  const signUpList = []
  const queueList = []
  const { logIn, hasRole, isAuthenticated, userMetadata, currentUser } =
    useAuth()
  ...
  if (hasRole('admin')) {
    for (var j = 0; signUps.length > j; j++) {
      ...
      guestPresent[signUpUser.user.id] = guestPresent[signUpUser.user.id]
        ? guestPresent[signUpUser.user.id]++
        : 1
      countPresent[signUpUser.user.id] = countPresent[signUpUser.user.id]
        ? 0
        : 0
  ...
  `
  const [updateMember, { updateMemberloading, updateMembererror }] =
  ...
  let signedUpPlayers = (
    <>
      ...
      <div>
        <h4>
          Signed Up Players : <font color="green">{signUps.length}</font>
        </h4>
        {queueList.length > 0 && (
          <h4>
            Queued Up Players : <font color="green">{queueList.length}</font>
          </h4>
        )}
        <table>
          <thead>
          ...
          <tbody>
            {signUpList.map((signup, count) => (
              <tr key={signup.id}>
                <td>
                  {truncate(signup.user.name)}
                  {countPresent[parseInt(signup.user.id)] >= 1 && (
                    <> +{countPresent[parseInt(signup.user.id)]} Guest </>
                  )}
                  <p hidden> {countPresent[parseInt(signup.user.id)]++}</p>
                  {(currentUser.id === signup.user.id || hasRole('admin')) &&
                    guestPresent[parseInt(signup.user.id)] < 2 &&
                    ++guestPresent[signup.user.id] && (
                      <button
                ...
                <td>
                  ...
                  {hasRole('admin') && (
                    <OrderWidget
                      schedule={schedule}
                      marker={{ QUERY, count, currentSignUpId: signup.id }}
                ...
                <td>
                  {!hasRole('admin') &&
                    countPresent[parseInt(signup.user.id)] == 1 &&
                    signup.user.member && <>Member</>}
                  {!hasRole('admin') &&
                    countPresent[parseInt(signup.user.id)] == 1 &&
                    !signup.user.member && <>Non Member</>}
                  {hasRole('admin') &&
                    countPresent[parseInt(signup.user.id)] == 1 &&
                    !signup.user.member && (
                      <button
  ...
  let queuePlayers = (
    <>
      <div>
        <h2>
          Queued Up Players : <font color="blue">{queueList.length}</font>
        </h2>
          ...
          <tbody>
            {userGoodList.map((user) => (
              <tr key={user.id}>
                ...
                <td>
                  {!hasRole('admin') && user.member && <>Member</>}
                  {!hasRole('admin') && !user.member && <>Non Member</>}
